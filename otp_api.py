...
def do_login():
    ...
    try:
        ...
        for inp in soup0.find_all("input"):
            ...
            elif "answer" in ph: login_data[nm] = ans

        r1 = session.post(SIGNIN_URL, data=login_data, timeout=15)
        is_logged_in = "logout" in r1.text.lower() or "login" not in r1.url.lower()
        log.info(f"Login: {'✅ OK' if is_logged_in else '❌ FAIL'}")
        return is_logged_in
