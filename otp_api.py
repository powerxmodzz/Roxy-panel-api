...
# ─────────────────────────────────────────
#  STARTUP — Gunicorn + Direct dono ke liye
# ─────────────────────────────────────────

def startup():
    """Login + background thread — gunicorn bhi call karta hai"""
    print("╔══════════════════════════════════╗")
    ...
    # Login with retry
    for i in range(3):
        print(f"Login attempt {i+1}/3...")
        if do_login():
            break
        time.sleep(3)

    if not is_logged_in:
        print("❌ Login fail! Credentials check karo.")
    else:
        # First fetch
        fetch_otps()

    # Background thread — gunicorn ke saath bhi chale
    t = threading.Thread(target=background_refresh, daemon=True)
    t.start()
    print(f"✅ Background refresh thread started (har {REFRESH_INTERVAL}s)")

# Module load hote hi startup — gunicorn ke liye
startup()
